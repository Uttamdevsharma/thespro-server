...
const getResearchCells = async (req, res) => {
  try {
    const researchCells = await ResearchCell.find({});
...
const addResearchCell = async (req, res) => {
  ...
  const cellExists = await ResearchCell.findOne({ title });

  if (cellExists) {
    res.status(400).json({ message: 'Research cell with this title already exists' });
    return;
  }
